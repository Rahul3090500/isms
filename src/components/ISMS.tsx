...
import classes from "../components/ISMS.module.scss";
...
interface MyAppProps {
  ...
  classificationChartData?: any; // Consider defining a more specific type
  classificationComments?: any[]; // Consider defining a more specific type
  loadingCommentClassifications?: boolean;
  loadingSentimentAnalysis?: boolean;
  loadingVideoSummary?: boolean;
  handleCommentClassifications?: () => void;
  rowData?: any;
  setIsFileOpener?: any;
  isFileOpener?: any;
}
...
function ISMS({ Component, pageProps }: any) {
  ...
  const updateCommentClassificationsChartData = async () => {
    ...
    try {
      ...
      setclassificationChartData((prevState: { datasets: any[] }) => ({
        ...prevState,
        datasets: prevState.datasets.map((dataset: any) => ({
          ...dataset,
          data: [
  ...
  //@ts-ignore
  const [selectedContent, setSelectedContent] = useState(navItems[0].text);
  const handleItemClick = (content: any) => {
    setSelectedContent(content);
    // handleSentimentAnalysis();
  };

  const additionalProps: MyAppProps = {
    handleOnChange,
    clear,
    isButtonLoading,
    handleSubmit,
